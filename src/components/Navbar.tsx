...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <div className="w-full max-w-[95rem] px-6 md:px-12 flex items-center justify-between">

                    {/* DESKTOP LEFT LINKS */}
                    <div className="hidden lg:flex items-center gap-3 xl:gap-5 w-[42%] justify-start flex-wrap">
                        {LEFT_ITEMS.map((item, i) => (
                            <a key={i} href={item.href} onClick={(e) => handleNavClick(e, item.href)} className="relative group py-2 cursor-pointer">
                                <span className="text-[9px] xl:text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400 group-hover:text-white transition-colors duration-300">
                                    {item.label}
                                </span>
                                <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-[#D2042D] transition-all duration-300 ease-out group-hover:w-full" />
                            </a>
                        ))}
                    </div>

                    {/* CENTER LOGO */}
                    <div className="w-[16%] flex justify-start lg:justify-center items-center z-[110] shrink-0">
                        <a
                            href="#home"
                            onClick={(e) => handleNavClick(e, "#home")}
                            className="relative group flex items-center justify-center text-white hover:text-[#D2042D] transition-colors duration-500 cursor-pointer"
                        >
                            <svg
                                className="w-16 md:w-20 lg:w-24 h-auto group-hover:scale-105 transition-transform duration-500 ease-out drop-shadow-md"
                    ...
                    {/* DESKTOP RIGHT LINKS */}
                    <div className="hidden lg:flex items-center gap-3 xl:gap-5 w-[42%] justify-end flex-wrap">
                    ...
                    <div className="w-[42%] flex justify-end lg:hidden z-[110]">
                        <button
                            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                            className="w-10 h-10 flex flex-col items-center justify-center gap-[5px] relative group"
                        >
                            <motion.span
                                animate={mobileMenuOpen ? { rotate: 45, y: 7 } : { rotate: 0, y: 0 }}
                                className="w-6 h-[2px] bg-white block transition-all duration-300 group-hover:bg-[#D2042D]"
                            />
                            <motion.span
